...
@objective(model, Min, ∫(A * S*I/1000+ u^2, t))
...
function loss_adjoint(θ)
    x = predict_adjoint(θ)
    mean(abs, A .* x[3, :].*x[1,:]/1000.0) + mean(abs2, [first(ann([t], θ)) for t in ts])
end
l = loss_adjoint(θ)
cb = function (θ, l)
    println(l)
...
results_saving_opt.u_nn = [first(ann([t], res2.u)) for t in ts]
...
xlabel!("t(day)")
ylabel!("I(t)")
savefig("output/sein_I.png")

plot(results_saving_opt.t[2:end-1], results_saving_opt.u[2:end-1], label = "u(t) by direct collection")
plot!(results_saving_opt.t[2:end-1], results_saving_opt.u_nn[2:end-1],  foreground_color_legend=nothing, label = "u(t) by deep learning")
xlabel!("t(day)")
ylabel!("u(t)")
savefig("output/sein_control.png")

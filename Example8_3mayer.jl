...
# infinite_parameter
@infinite_parameter(model, t ∈ [t0, tf], num_supports = 401)
# variable
...
cb = function (θ, l)
    println(l)
    p = plot(Array(solveeq(remake(prob, p = θ), Tsit5(), saveat = ts))[1, :], lw = 3)
...
CSV.write("results_saving_Example8_3_mayer.csv", results_saving_opt)
...
xlabel!("t(day)")
ylabel!("I(t)")
savefig("Example8_3_I_mayer.png")
...
xlabel!("t(day)")
ylabel!("u(t)")
savefig("Example8_3_control_mayer.png")
